...
//  Guard : si window.supabase n'est pas encore dispo (CDN lent), on attend
// ═══════════════════════════════════════════════════════════════════════════
if (!window.supabase) {
  console.error('[Supabase] SDK non chargé ! Vérifiez le CDN dans index.html');
  // Créer un stub pour éviter les crashs en cascade
  window.supabase = {
    createClient: () => ({
      from: () => ({ select:()=>Promise.resolve({data:null,error:{message:'SDK manquant'}}), insert:()=>Promise.resolve({error:{message:'SDK manquant'}}), update:()=>Promise.resolve({error:{message:'SDK manquant'}}), delete:()=>Promise.resolve({error:{message:'SDK manquant'}}), upsert:()=>Promise.resolve({error:{message:'SDK manquant'}}), eq:function(){return this;}, single:()=>Promise.resolve({data:null,error:{message:'SDK manquant'}}) }),
      rpc: () => Promise.resolve({ data: null, error: { message: 'Supabase SDK non chargé. Vérifiez le CDN.' } }),
      channel: () => ({ on: function(){ return this; }, subscribe: function(cb){ if(cb) cb('CHANNEL_ERROR'); return this; }, send: ()=>Promise.resolve(), }),
...
// ═══════════════════════════════════════════════════════════════════════════
//  REMPLACEMENT RoomManager / WS / SyncManager
//  Ces objets n'existent qu'APRÈS initPhase3() (déclenché par btn-enter).
//  On attend leur présence avant d'appliquer les overrides.
// ═══════════════════════════════════════════════════════════════════════════
function _applyOverrides() {
  if (window.RoomManager) {
    ...
    console.info('[Supabase] RoomManager overridé ✓');
  }
  ...
  if (window.WS) {
    WS.send = (type, payload) => {
      switch (type) {
        case 'MOVE_TOKEN':        sbMoveToken(payload.tokenId, payload.x, payload.y); break;
        case 'UPSERT_TOKEN':      if (payload.token) sbUpsertToken(payload.token); break;
        case 'DELETE_TOKEN':      if (payload.tokenId) sbDeleteToken(payload.tokenId); break;
        case 'ASSIGN_TOKEN_OWNER': sbAssignTokenOwner(payload.tokenId, payload.ownerPlayerId); break;
        case 'UPDATE_TOKEN_HP':   sbUpdateTokenHp(payload.tokenId, payload.hp); break;
        case 'UPDATE_PLAYER':     if (payload.player) sbUpdatePlayer(payload.player); break;
        case 'CHANGE_SCENE':      if (payload.sceneId) sbChangeScene(payload.sceneId); break;
        case 'UPDATE_FOG':        sbUpdateFog(payload.fogData, payload.fogEnabled); break;
        case 'UPDATE_COMBAT':     sbUpdateCombat(payload); break;
        case 'SEND_CHAT':         if (payload.text) sbSendChat(payload.text); break;
        case 'PLACE_MARKER':      if (payload.marker) sbPlaceMarker(payload.marker); break;
        case 'CLOSE_ROOM':        sbCloseRoom(); break;
        // Ignorer silencieusement les types WS anciens non pertinents
        case 'PING': case 'CREATE_ROOM': case 'JOIN_ROOM': case 'REJOIN_ROOM': break;
        default: break;
      }
    };
    WS.isConnected = () => !!SbState.roomId;
    // Neutraliser la connexion WS native — on passe par Supabase
    WS.connect = (url) => {
      console.info('[Supabase] WS.connect() intercepté — connexion Supabase à la place');
    };
    console.info('[Supabase] WS overridé ✓');
  }
}

// Appliquer dès que les objets sont disponibles (après initPhase3)
(function _waitForOverride() {
  const _t = setInterval(() => {
    if (document.getElementById('app')?.classList.contains('hidden')) return;
    if (!window.RoomManager || !window.WS) return;
    clearInterval(_t);
    _applyOverrides();
  }, 80);
})();
...
function _sbInit() {
  ...
  // Hooks après initApp() — on attend que State ET que l'app soit rendue
  // NE PAS s'exécuter sur le splash screen, seulement après enterApp()
  const _waitApp = setInterval(() => {
    // State est défini dès script.js, mais initApp() est appelé après btn-enter
    // On détecte que l'app est prête quand #app n'a plus la classe 'hidden'
    if (!window.State) return;
    if (document.getElementById('app')?.classList.contains('hidden')) return;
    ...
    // Reconnexion différée — ne pas bloquer l'UI
    setTimeout(() => {
      sbReconnect().then(ok => {
