...
function initApp() {
  loadFromStorage();
  const savedPId = localStorage.getItem('co-player-id');
  if (savedPId && !State.playerId) State.playerId = savedPId;
  else if (!State.playerId) State.playerId = 'player-' + Date.now() + '-' + Math.floor(Math.random()*9999);
  applySavedSettings();
...
function rollDie(sides) {
  ...
  // Broadcaster le lancer de dé au chat en mode multijoueur
  if (typeof WS !== 'undefined' && WS.isConnected() && RoomManager.room) {
    WS.send('SEND_CHAT', { text: `🎲 ${msg}` });
  }
}
...
// Envoie l'état du combat au serveur (appelé par nextTurn, nextRound, et les hooks CombatManager)
function _syncCombatState() {
  if (typeof WS !== 'undefined' && WS.isConnected() && RoomManager.room && State.playerRole === 'mj') {
...
// ─── TOUCH TOKEN DRAG — Support mobile ────────────────────────────────────────
function setupTouchTokenDrag() {
  const vp = $('map-viewport');
  if (!vp) return;

  // Map chaque touch identifier à son état de drag
  const touchState = new Map(); // touchId → { tokenEl, tokenId, startX, startY, dragging, offX, offY }
  ...
  vp.addEventListener('touchstart', (e) => {
    ...
    for (const touch of e.changedTouches) {
      ...
      if (!tokenEl || !vp.contains(tokenEl)) continue;

      e.preventDefault();
      const tokenId = tokenEl.dataset.tokenId;

      // Vérification permission joueur
      if (State.playerRole === 'joueur') {
        ...
        if (!canControl) { showToast('Ce personnage ne vous appartient pas', 'warning', '⚠'); continue; }
      }

      selectToken(tokenId);
      const rect = vp.getBoundingClientRect();
      const wx = (touch.clientX - rect.left - State.panX) / State.zoom;
      const wy = (touch.clientY - rect.top  - State.panY) / State.zoom;
      const t = State.tokens.find(t => t.id === tokenId);
  ...
  vp.addEventListener('touchmove', (e) => {
    for (const touch of e.changedTouches) {
      const ts = touchState.get(touch.identifier);
      if (!ts) continue;
      e.preventDefault();
      ...
      if (!ts.dragging && Math.hypot(dx, dy) >= 5) {
        ts.dragging = true;
        ts.tokenEl.classList.add('dragging');
        // Suspend pan
        State.isPanning = false;
      }

      if (ts.dragging) {
        const wx = (touch.clientX - rect.left - State.panX) / State.zoom - ts.offX;
        const wy = (touch.clientY - rect.top  - State.panY) / State.zoom - ts.offY;
        moveTokenEl(ts.tokenId, wx, wy, false);
      }
    }
  }, { passive: false });
  ...
  const endTouch = (e) => {
    for (const touch of e.changedTouches) {
      ...
      if (!ts) continue;
      if (ts.dragging) {
        ts.tokenEl.classList.remove('dragging');
        const t = State.tokens.find(t => t.id === ts.tokenId);
        if (t) moveTokenEl(ts.tokenId, t.x, t.y, true);
        saveToStorage();
      }
      touchState.delete(touch.identifier);
    }
  };
  vp.addEventListener('touchend',    endTouch, { passive: false });
  vp.addEventListener('touchcancel', endTouch, { passive: false });
}
...
const RoomManager = {
  ...
  createRoom(name, gmName, maxPlayers = 4) {
    ...
    const gmId = State.playerId || ('player-mj-' + Date.now() + '-' + Math.floor(Math.random()*9999));
    State.playerId = gmId;
    State.playerRole = 'mj';
    try { localStorage.setItem('co-player-id',   gmId); } catch(_) {}
    try { localStorage.setItem('co-player-name',  gmName || 'Maître du Jeu'); } catch(_) {}
...
function bindNetworkEvents() {
  ...
  WS.on('ROOM_CREATED', (data) => {
    ...
    PlayersPanel.render();
    // Persister pour reconnexion auto
    try { localStorage.setItem('co-saved-room', JSON.stringify({ code: data.roomCode, playerId: State.playerId })); } catch(_) {}
    try { localStorage.setItem('co-player-role', 'mj'); } catch(_) {}
    try { localStorage.setItem('co-player-name', RoomManager.room.gmName || 'Maître du Jeu'); } catch(_) {}
  ...
  WS.on('ROOM_JOINED', (data) => {
    ...
    // Mettre à jour le code salle côté client
    if (RoomManager.room && data.snapshot.roomCode) {
      RoomManager.room.code = data.snapshot.roomCode;
      RoomManager.room.name = RoomManager.room.name || data.snapshot.roomCode;
      RoomManager.room.players = data.snapshot.players || RoomManager.room.players;
    }
    ...
    ChatUI.renderAll();
    // Persister pour reconnexion auto
    if (data.snapshot.roomCode) {
      try { localStorage.setItem('co-saved-room', JSON.stringify({ code: data.snapshot.roomCode, playerId: State.playerId })); } catch(_) {}
      try { localStorage.setItem('co-player-role', State.playerRole); } catch(_) {}
    }
  ...
  WS.on('SNAPSHOT', (data) => {
    if (!data.snapshot) return;
    // Restaurer la room si nécessaire (reconnexion)
    if (data.snapshot.roomCode && !RoomManager.room) {
      const savedRole = localStorage.getItem('co-player-role') || 'joueur';
      const savedName = localStorage.getItem('co-player-name') || 'Joueur';
      State.playerRole = savedRole;
      RoomManager.room = {
        code: data.snapshot.roomCode,
        id: 'room-reconnected',
        name: data.snapshot.roomCode,
        gmId: data.snapshot.hostPlayerId,
        gmName: 'Maître du Jeu',
        players: data.snapshot.players || [],
        maxPlayers: 8,
      };
      if (savedRole === 'joueur') RoomManager._applyPlayerMode();
      else RoomManager._applyGMMode();
    } else if (data.snapshot.roomCode && RoomManager.room) {
      RoomManager.room.code    = data.snapshot.roomCode;
      RoomManager.room.players = data.snapshot.players || RoomManager.room.players;
    }
    Session.applySnapshot(data.snapshot);
    ...
    updateLog('État restauré depuis le serveur');
  });

  // ── Salle fermée par le MJ ────────────────────────────────────────────
  WS.on('ROOM_CLOSED', (data) => {
    showToast(data.message || 'La salle a été fermée', 'warning', '🌐');
    RoomManager.room = null;
    State.playerRole = 'mj';
    RoomManager._applyGMMode();
    RoomManager._renderRoomPanel(false);
    RoomManager._updateSessionIndicator(false);
    _setSessionStatus('offline');
    try { localStorage.removeItem('co-saved-room'); } catch(_) {}
  });
...
function initPhase3() {
  ...
  // 11. Auto-connect WS si URL sauvegardée ET reconnexion salle
  const savedUrl  = localStorage.getItem('co-ws-url');
  const savedRoom = localStorage.getItem('co-saved-room');
  if (savedUrl) {
    setTimeout(() => {
      const input = $('ws-url-input');
      if (input) input.value = savedUrl;
      // Auto-connecter si URL sauvegardée
      _setWSStatus('connecting', 'Auto-connexion…');
      WS.connect(savedUrl);
      // Si une salle était en cours, tenter de la rejoindre après connexion
      if (savedRoom) {
        try {
          const roomInfo = JSON.parse(savedRoom);
          const onConnected = () => {
            WS.off('ws:connected', onConnected);
            if (!RoomManager.room && roomInfo.code && roomInfo.playerId) {
              const savedName = localStorage.getItem('co-player-name') || 'Joueur';
              const savedRole = localStorage.getItem('co-player-role') || 'joueur';
              State.playerId  = roomInfo.playerId;
              State.playerRole = savedRole;
              WS.send('REJOIN_ROOM', {
          ...
          WS.on('ws:connected', onConnected);
        } catch(_) {}
      }
    }, 800);
  }
...
function setupMultiplayer() {
  ...
  $('btn-confirm-join-room')?.addEventListener('click', () => {
    ...
    if (!State.playerId) State.playerId = 'player-' + Date.now() + '-' + Math.floor(Math.random()*9999);
    try { localStorage.setItem('co-player-name', name); } catch(_) {}
    try { localStorage.setItem('co-player-id',   State.playerId); } catch(_) {}
