...
/* ── PATCH #0 : Bouton "Entrer dans la Taverne" ────────────────────────────
   Attache le listener dès que le DOM est prêt.
   Si script.js a déjà attaché le sien, ce handler redondant est inoffensif
   (initApp est idempotente). Si script.js a planté, c'est ce handler qui prend
   le relais et déclenche initApp() manuellement.
   ─────────────────────────────────────────────────────────────────────────── */
(function patchBtnEnter() {
  function _attachEnter() {
    const btn = document.getElementById('btn-enter');
    if (!btn) return;

    // Éviter le double-bind : on marque le bouton
    if (btn._patchBound) return;
    btn._patchBound = true;

    btn.addEventListener('click', () => {
      // Si script.js expose initApp(), l'appeler
      if (typeof window.initApp === 'function') {
        window.initApp();
        return;
      }
      // Fallback minimal : basculer splash → app
      const splash = document.getElementById('splash-screen');
      const app    = document.getElementById('app');
      if (splash) splash.style.display = 'none';
      if (app)    app.classList.remove('hidden');
      console.info('[Patch#0] initApp() non trouvée — bascule splash→app manuelle');
    });

    console.info('[Patch#0] btn-enter ✓');
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _attachEnter);
  } else {
    _attachEnter();
  }
})();
...
(function applySupabasePatch() {
  ...
  // WS, RoomManager, State existent seulement APRÈS que l'utilisateur clique sur
  // "Entrer dans la Taverne" → initApp() → setupMultiplayer() → initPhase3()
  // On doit donc attendre que l'app soit lancée ET que ces objets soient initialisés
  const _waitReady = setInterval(() => {
    // 1. L'app doit être visible (splash disparu)
    if (document.getElementById('app')?.classList.contains('hidden')) return;
    // 2. State, RoomManager et WS doivent exister
    if (!window.State || !window.RoomManager || !window.WS) return;
    // 3. ChatUI doit être initialisé (via initPhase3 → ChatUI.init())
  ...
  function _patch() {
    ...
    // ── 1. Neutraliser immédiatement le panneau WS ──────────────────────────
    //    _injectWSPanel() dans initPhase3 crée un champ de saisie URL WebSocket.
    //    On le remplace par une bannière Supabase DÈS que c'est possible.
    //    On tente aussi de prévenir que quelqu'un ne clique "Connecter" avant.
    ...
    // Essayer immédiatement, puis retry si le panneau n'est pas encore injecté
    if (!_replaceWSPanel()) {
      const _wsRetry = setInterval(() => { if (_replaceWSPanel()) clearInterval(_wsRetry); }, 100);
      setTimeout(() => clearInterval(_wsRetry), 3000);
    }

    // Neutraliser WS.connect pour qu'il ne tente JAMAIS une vraie connexion WS
    if (window.WS) {
      WS.connect = () => console.info('[Patch] WS.connect() bloqué — on utilise Supabase');
    }
